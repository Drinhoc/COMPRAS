...
def filter_required_fields(df: pd.DataFrame) -> pd.DataFrame:
    if {"item", "data_solicitacao", "empresa"}.isdisjoint(df.columns):
        return df
    ...
    empresa_series = df.get("empresa").astype(str) if "empresa" in df.columns else None
    ...
    if empresa_series is not None:
        mask &= df["empresa"].notna() & empresa_series.str.strip().ne("")
